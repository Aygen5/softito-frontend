...
const MultipleInputs = () => {
  ...
  const handleInputChange = (e) => {
    ...
    const guncelDeger = type === "checkbox" ? checked : type === "number" ? (value === "" ? "" : Number(value)) : value;

    setFormVerileri(prev => ({...prev, [name]:guncelDeger }));
  };
  ...
  return (
    <div className="p-4">
      ...
      <p className="demo-desc"> Formda çok sayıda girdi varsa, her birine ayrı <code>useState</code> açmak yerine tek bir nesne state'i ve girdilerin <code>name</code> özellikleri kullanılarak dinamik yönetim sağlanır. </p>

      <div className="demo-card-layout-grid max-w-3xl"> {/* Dinamik Form */}
        <div className="demo-card demo-card-3xl space-y-4">
          ...
          <button onClick={formuSifirla} className="btn-gray-full mt-2">Formu Temizle</button>
        </div>
        ...
        <div className="demo-card demo-card-3xl demo-amber-card">
          <div>
            <span className="badge-amber-mini">Canlı State İzleyici</span>
            <h4 className="demo-section-title">JSON Formatında State Durumu:</h4>
            <pre className="demo-pre-amber"> {JSON.stringify(formVerileri, null, 2)}</pre>
          </div>
