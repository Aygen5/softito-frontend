...
const DynamicFormFields = () => {
  ...
  const yetenekEkle = () => {
    setYetenekler([...yetenekler, { id: Date.now(), ad: "", seviye: "Başlangıç" } ]);
  };
  ...
  const yetenekGuncelle = (id, field, value) => {
    setYetenekler( yetenekler.map(y => y.id === id ? { ...y, [field]: value } : y)
    );
  };
  ...
  return (
    <div className="p-4">
      ...
      <p className="demo-desc"> Kullanıcının form üzerinde dinamik olarak yeni satırlar eklemesine veya silmesine imkan tanımak için dizi state'leri ile form girdileri map edilerek yönetilir.</p>
      ...
      <div className="demo-card demo-card-2xl">
        <div className="card-header-list pb-2 mb-4">
          ...
          <button type="button" onClick={yetenekEkle} className="btn-blue-outline-xs">+ Yeni Yetenek Ekle</button>
        </div>


        <form onSubmit={handleSubmit} className="space-y-3"> {yetenekler.map((y, index) => (
            <div key={y.id} className="demo-dyn-row">
              ...
              <button type="button" onClick={() => yetenekSil(y.id)} className="btn-delete-dyn">Sil </button>
            </div>
          ))}

          <div className="demo-form-actions-row">
            <button type="submit" className="btn-blue-submit-lg">Yetenekleri Kaydet</button>
          </div>
        ...
        <div className="demo-code-footer mt-4 pt-3">
          ...
          <pre className="demo-pre-dark">{JSON.stringify(yetenekler, null, 2)}</pre>
        </div>
