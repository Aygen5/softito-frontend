/* sepetim butonuna basınca açılan panel*/ 

/* useMemo = bu hesaplama pahalıysa, gereksiz yere tekrar yapma demektir*/
...
export default function SepetGezgini({
    ...
    onUrunCikar
     ...
     /* ürünlerin toplam fiyatı hesaplanır.*/
    const toplamFiyat = useMemo(() => {
        return sepet.reduce((toplam, item) => toplam + item.fiyat * item.adet, 0);
    }, [sepet]); /* sepet değişmediyse toplamFiyat yeniden hesapanmaz. bu useMemo'nun amacı.*/ 


    /* kargo sistemi*/ 
    const kargoLimit = 1500;

    /* örnek: 2000TL ise kargo bedava. 1000 TL ise kargo 50 TL.*/
    ...
    const ilerlemeYuzdesi = Math.min((toplamFiyat / kargoLimit) * 100, 100);
     ...
     /* eğer isOpen=true ise visible. eğer false ise hidden. css animasyonu içindir.*/
    const drawerClass = `sepet-drawer ${isOpen ? "sepet-drawer-visible" : "sepet-drawer-hidden"}`;


    if (!isOpen) return null;  /* eğer sepet kapalıysa hiç render etme. boş dön*/
