/* kullanıcı ürün kartına tıklıyor. 
   çıkan pencereye modal denir.
   buradaki component ile o modal oluşturuluyor.*/
...
/* 3 prop geliyor. 
    ...
    onClose= modalı kapatacak fonksiyon,
    onSepeteEkle= ürünü sepete ekleyecek fonksiyon */
export default function UrunDetayi({ product, onClose, onSepeteEkle }) {

  useEffect(() => {
    const handleKeyDown = (e) => {
      if (e.key === "Escape") {
        onClose();  /*escape ile modal kapanır*/
      }
    };
    window.addEventListener("keydown", handleKeyDown); /* klavyeyi dinle. her tuşa bastığında handleKeyDown çalışsın*/

    
    return () => {
      window.removeEventListener("keydown", handleKeyDown);
    };
  }, [onClose]); /* dependency array: onClose değişirse effect yeniden çalıştır*/



   /* normalde sayfa aşağı yukarı kaydırılabilir.
      ama modal açıldığında arka plan kaymasın istiyoruz.
      overflow:hidden ile scroll kapanır.*/
  useEffect(() => {
    document.body.style.overflow = "hidden";
    
    return () => {
      document.body.style.overflow = "";  /* modal kapanınca overflow kaldırılır. scroll geri gelir.*/ 
    };
  }, []);
  ...
  useEffect(() => {
    if (product) { /* model ilk açıldığında çalışır.*/
      console.log(`[Ders Notu - Mount] UrunDetayi modalı açıldı: ${product.ad}`);
    }
    return () => { /* modal kapatılırken çalışır.*/ 
      console.log("[Ders Notu - Unmount] UrunDetayi modalı kapatıldı ve bellekten temizlendi.");
    };
  }, [product]);

  /* mount: component oluşturuldu
     yoktu-> oluşturuldu->mount */

  /* unmount: component silindi
     vardı-> silindi-> unmount */




    /* ürün stok kontrolü yapılıyor */ 
  ...
  if (!product) return null;  /* product = null ise hiçbir şey render yetme.
                                 yani modal gösterme.*/ 
